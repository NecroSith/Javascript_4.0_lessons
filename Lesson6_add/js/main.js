...
for (let i = 0; i < optExpensesItems.length; i++) {
    optExpensesItems[i].setAttribute("oninput","if(!/^[а-яА-Я]*$/g.test(this.value)) {this. value = ''} else {this.value = this.value}");
...
buttonApproveOpt[0].addEventListener('click', function() {
    optExpensesValue.textContent = '';
    for (let i = 0; i < optExpensesItems.length; i++) {
        let point = optExpensesItems[i].value.replace(/[^0-9a-яёъ]/ig, '');
